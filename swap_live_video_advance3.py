...
class FaceSwapApp:
    ...
    def setup_ui(self):
        ...
        # === Image Frames ===
        self.image_frame = Frame(main_frame, bg="#f0f2f5")
        self.image_frame.pack(fill=BOTH, expand=True)

        self.source_frame = LabelFrame(self.image_frame, text="Source Image", font=("Arial", 11, "bold"), bg="white")
        self.source_frame.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
        self.source_label = Label(self.source_frame, bg="white")
        self.source_label.pack(padx=10, pady=10)

        self.target_frame = LabelFrame(self.image_frame, text="Target Image", font=("Arial", 11, "bold"), bg="white")
        self.target_frame.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")
        self.target_label = Label(self.target_frame, bg="white")
        self.target_label.pack(padx=10, pady=10)

        self.result_frame = LabelFrame(self.image_frame, text="Result Image", font=("Arial", 11, "bold"), bg="white")
        self.result_label = Label(self.result_frame, bg="white")
        self.result_label.pack(padx=10, pady=10)

        # === Control Panel ===
        control_frame = Frame(main_frame, bg="#f0f2f5")
        control_frame.pack(fill=X, pady=10)

        # Sol taraftaki butonlar
        left_buttons = Frame(control_frame, bg="#f0f2f5")
        left_buttons.pack(side=LEFT, padx=10)

        self.make_button(left_buttons, "Load Source", self.load_source).pack(side=LEFT, padx=5)
        self.make_button(left_buttons, "Load Target", self.load_target).pack(side=LEFT, padx=5)
        self.make_button(left_buttons, "Generate AI Face", self.generate_ai_face).pack(side=LEFT, padx=5)
        self.make_button(left_buttons, "Webcam Source", lambda: self.capture_from_webcam(is_source=True)).pack(
            side=LEFT, padx=5)
        self.make_button(left_buttons, "Webcam Target", lambda: self.capture_from_webcam(is_source=False)).pack(
            side=LEFT, padx=5)
        self.make_button(left_buttons, "Live Swap Video", self.open_live_video).pack(side=LEFT, padx=5)

        # Sağ taraftaki butonlar
        right_buttons = Frame(control_frame, bg="#f0f2f5")
        right_buttons.pack(side=RIGHT, padx=10)

        self.email_button = self.make_button(right_buttons, "Email Result", self.email_result, "#008CBA")
        self.email_button.pack(side=RIGHT, padx=5)
        self.email_button.config(state=DISABLED)

        self.swap_button = self.make_button(right_buttons, "Swap Faces", self.swap_faces, "#ff9800")
        self.swap_button.pack(side=RIGHT, padx=5)

        self.save_button = self.make_button(right_buttons, "Save Result", self.save_result, "#4CAF50")
        self.save_button.pack(side=RIGHT, padx=5)
        self.save_button.config(state=DISABLED)

        # === Settings ===
        settings_frame = Frame(main_frame, bg="#f0f2f5")
        settings_frame.pack(fill=X, pady=5)

        Label(settings_frame, text="Blend Amount:", bg="#f0f2f5", font=("Arial", 10)).grid(row=0, column=0)
        self.blend_scale = Scale(settings_frame, from_=0, to=100, orient=HORIZONTAL, length=180, bg="#f0f2f5")
        self.blend_scale.set(65)
        self.blend_scale.grid(row=0, column=1)
        self.blend_scale.bind("<ButtonRelease-1>", self.update_blend)

        Label(settings_frame, text="Color Adjustment:", bg="#f0f2f5", font=("Arial", 10)).grid(row=0, column=2)
        self.color_scale = Scale(settings_frame, from_=0, to=100, orient=HORIZONTAL, length=180, bg="#f0f2f5")
    ...
    def make_button(self, parent, text, command, color="#2196F3"):
        return Button(
            parent,
            text=text,
            command=command,
